...
case class UsZipCode(deliverable: Boolean,
...
object UsZipCode {
  ...
  def getUsZipCode(json: JsObject): UsZipCode = {
    ...
    val deliverable =
      jsonHelper.getBooleanValue(DELIVERABLE_FIELD).getOrElse(false)
    val exactMatch =
      jsonHelper.getBooleanValue(EXACT_COMPANY_MATCH_FIELD).getOrElse(false)
    val formExcepts =
      jsonHelper.getDefaultValue(FORM_EXCEPTS_FIELD).getOrElse("")
